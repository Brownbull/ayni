...
class TestDetailedHealthEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_detailed_health_redis_placeholder(self, client: TestClient):
        """Test detailed health endpoint includes Redis status (placeholder)"""
        response = client.get("/api/v1/health")
        data = response.json()

        # Redis should be noted as not configured (Story 1.4 will add it)
